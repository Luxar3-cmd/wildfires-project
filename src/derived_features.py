...
def kelvin_to_celsius(t_k: pd.Series) -> pd.Series:
	"""Convierte una serie de temperaturas de Kelvin a grados Celsius.

	Args:
		t_k: Temperaturas en Kelvin (se coercionan a numérico).
...
def relative_humidity(t2m_k: pd.Series, d2m_k: pd.Series) -> pd.Series:
	"""Calcula la humedad relativa (%) desde temperatura y punto de rocío en Kelvin.
	...
	Args:
		t2m_k: Temperatura del aire a 2 m (t2m) en Kelvin.
		d2m_k: Temperatura del punto de rocío a 2 m (d2m) en Kelvin.

	Returns:
		Humedad relativa (RH) en porcentaje, recortada al rango [0, 100].
	"""
...
def wind_speed(u: pd.Series, v: pd.Series) -> pd.Series:
	"""Calcula la magnitud del viento (m/s) desde las componentes u, v.

	Args:
		u: Componente zonal del viento (u10) en m/s.
		v: Componente meridional del viento (v10) en m/s.
...
def wind_direction(u: pd.Series, v: pd.Series) -> pd.Series:
	"""Calcula la dirección meteorológica del viento (grados desde donde sopla).

	Convención meteorológica: indica de dónde proviene el viento, con 0=N, 90=E.
	...
	Returns:
		Dirección del viento en grados, en el rango [0, 360).
	"""
	u = pd.to_numeric(u, errors="coerce")
	v = pd.to_numeric(v, errors="coerce")
...
def precipitation_mm(tp_m: pd.Series) -> pd.Series:
	"""Convierte la precipitación total de metros a milímetros.

	ERA5 entrega tp (total precipitation) en metros acumulados por hora.

	Args:
		tp_m: Precipitación total (tp) en metros.

	Returns:
		Precipitación total en milímetros.
	"""
	return pd.to_numeric(tp_m, errors="coerce") * 1000.0


def vpd_hpa(t2m_k: pd.Series, d2m_k: pd.Series) -> pd.Series:
	"""Calcula el Déficit de Presión de Vapor (VPD, hPa) desde temperatura y rocío en Kelvin.
	...
	Returns:
		VPD en hPa, recortado a valores no negativos.
	"""
	t = pd.to_numeric(t2m_k, errors="coerce") - KELVIN_TO_C
	td = pd.to_numeric(d2m_k, errors="coerce") - KELVIN_TO_C
	es_t = 6.1094 * np.exp(17.625 * t / (t + 243.04))
...
def add_all(df: pd.DataFrame) -> pd.DataFrame:
	...
	Args:
		df: DataFrame con las variables crudas de ERA5 como columnas.

	Returns:
		Copia del DataFrame con las columnas derivadas añadidas (t2m_celsius,
		relative_humidity, vpd_hpa, wind_speed, wind_direction, tp_mm, stl{1-4}_celsius),
		según qué columnas base estén disponibles.
	"""
	df = df.copy()
